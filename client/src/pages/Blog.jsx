...
const Blog = () => {
  ...
  return data ? (
    <div className='relative'>
        <Navbar />
       <img src={assets.gradientBackground} alt="" className='absolute -z-1 -top-50 opacity-50'/>
      <div className='text-center text-gray-600 mt-20'>
        <p className='text-primary py-4 font-medium'>Published on {Moment(data.createdAt).format("MMMM Do YYYY")}</p>
        <h1 className='text-2xl sm:text-5xl font-semibold max-w-2xl mx-auto text-gray-800'>{data.title}</h1>
        <h2 className='my-5 truncate max-w-lg mx-auto'>{data.subTitle}</h2>
        <p className='inline-block py-1 px-4 rounded-full mb-6  border text-sm border-primary/35 bg-primary/5  font-medium text-primary'>Stephen</p>
      </div>
...
{/* add comment section */}
        ...
        <form onSubmit={addComment} className='flex flex-col items-start gap-4 max-w-lg'>
            <input value={name} onChange={(e) => setName(e.target.value)} type="text" placeholder='Name' className='w-full p-2 border border-gray-300 rounded outline-none'/>
